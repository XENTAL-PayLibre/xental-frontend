...
const MOCK_DATA: Record<string, { status: TxStatus; amount: string; customer: string; account: string; narration: string }> = {
  OUT202001: { status: 'Successful', amount: '₦25,000', customer: 'Chinonso Okeke', account: '8061782007', narration: 'Withdrawal' },
  OUT202002: { status: 'Successful', amount: '₦18,500', customer: 'Amara Nwosu', account: '2375849108', narration: 'Transfer' },
  OUT202003: { status: 'Pending', amount: '₦10,000', customer: 'Emeka Obi', account: '8061782008', narration: 'Settlement' },
  OUT202004: { status: 'Failed', amount: '₦45,000', customer: 'Fatima Bello', account: '4928375610', narration: 'Payout' },
};
...
export default function PayOutDetailPage({ params }: { params: Promise<{ ref: string }> }) {
  const { ref } = use(params);
  const data = MOCK_DATA[ref] ?? { status: 'Successful' as TxStatus, amount: '₦25,000', customer: 'Chinonso Okeke', account: '3024567891', narration: 'Withdrawal' };
    ...
    <div className='flex flex-col gap-5'>
      ...
      <div className='grid grid-cols-2 gap-4'>
        ...
        <div className='bg-white rounded-xl border border-stroke-2 p-5'>
          ...
          <p className='text-xs text-xental-text-primary-400 mb-4'>Details of this pay-out transaction</p>
          <DetailRow label='Transaction ID' value={`TXN${ref.replace('OUT', '')}`} valueClass='font-mono' />
          <DetailRow
            label='Status'
            value={
          ...
          <DetailRow label='Fee' value='₦50.00' />
