...
export const API_TAGS: TagMeta[] = [
  ...
  // Reconciliation & webhooks (the inbound provider receiver is internal — not documented here)
  ...
  // Auth (token exchange), key management, and onboarding all happen in the Xental
  // dashboard, so they are intentionally NOT documented as public sections here.
...
export function getDocsNav(): NavGroup[] {
  ...
  const order = ['Core', 'Reconciliation', 'Settlement', 'Real-time & automation', 'Platform'];
